...
namespace EDNET
{
    class PiezaL : Pieza
    {
        public PiezaL(Point posic, int separac, int tamCuad, GraphicsDeviceManager graphics) : base(posic, separac, tamCuad, graphics)
        {
            color = Color.Orange;
        }
        public override void rotaPieza()
        {
            switch (rotac)
            {
                case 1:
                    ...
                    cuadrados[3].Location = new Point(calcPos[2]((int)posic.X), calcPos[0]((int)posic.Y));
                    break;
                case 2:
                    for (int i = 0; i < 3; i++)
                    {
                    ...
                    cuadrados[3].Location = new Point(calcPos[1]((int)posic.X), calcPos[2]((int)posic.Y));
                    break;
                case 3:
                    for (int i = 0; i < 3; i++)
                    {
